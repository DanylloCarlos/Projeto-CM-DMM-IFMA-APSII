 ...
 * @author     Danyllo C.
 */
class DelOuAtEstoque extends TPage
{
    private $form;
    private $datagrid;
    private $loaded;
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle(_t('Gerenciar Estoque'));
        
        // create the form fields
        $product_id           = new TDBSeekButton('idItem', 'cr2m', 'form_pos', 'Item', 'nome');
      ...
      //  $product_id->setAuxiliar($product_description);
        ...
        $product_description->setSize(250);
        ...
        $lab_it = new TLabel('Item'); //lab_pro
     //   $lab_pri = new TLabel('Price');
        $lab_qtd = new TLabel('Quantidade'); //lab_amo
     //   $lab_tot = new TLabel('Total');
        $lab_it->setFontSize(17);
       // $lab_pri->setFontSize(17);
        $lab_it->setFontSize(17);
        $lab_qtd->setFontSize(17);
        $lab_it->setFontColor('blue');
        $lab_qtd->setFontColor('blue');
        $this->form->addField($product_description);
        
        // add the form fields
        $this->form->addFields([$lab_it], [$product_id]);
        $this->form->addFields([$lab_qtd], [$amount]);
        $this->form->addAction('Adicionar Item', $a1=new TAction(array($this, 'onAddItem')), 'fa:plus green');
        $this->form->addAction('Limpar',    $a2=new TAction(array($this, 'onClear')),   'fa:trash red');
        $this->form->addAction('Salvar',     $a3=new TAction(array($this, 'onCustomer')),'fa:save blue');
       ...
       // $this->datagrid->addQuickColumn('ID', 'product_id', 'right', 50);
        $this->datagrid->addQuickColumn('Nome', 'product_description', 'left');
        $this->datagrid->addQuickColumn('Descrição', 'sale_price', 'right');
        $this->datagrid->addQuickColumn('Quantidade', 'amount', 'right');
